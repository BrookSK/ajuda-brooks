...
class MobileController extends Controller
{
    ...
    /**
     * Endpoint SSE: gera resposta da IA com streaming token a token.
     * POST /m/chat/stream
     */
    public function sendMessageStream(): void
    {
        ...
        if ($conversationId <= 0 || $text === '') {
            ...
            echo "event: error\ndata: " . json_encode(['message' => 'Mensagem vazia.']) . "\n\n";
            return;
        }
        ...
        if (!$conv) {
            header('Content-Type: text/event-stream');
            echo "event: error\ndata: " . json_encode(['message' => 'Conversa não encontrada.']) . "\n\n";
            return;
        }
        ...
        // Carrega contexto (mesmo do sendMessage)
        $onboarding = UserOnboarding::findByUserId($userId);
        $personaId = $conv['persona_id'] ?? ($onboarding['personality_id'] ?? null);
        $persona = $personaId ? Personality::findById((int)$personaId) : null;
        $convSettings = ConversationSetting::findForConversation($conversationId, $userId);
        ...
        $engine = new TuquinhaEngine();
        $engine->setAiLearnings($learnings);
        $result = $engine->streamResponseWithContext($history, null, $userContext, $convSettings, $persona);
        ...
        if ($result && isset($result['content'])) {
            ...
            // Auto-título na primeira mensagem (assíncrono — não bloqueia)
            if (count($allMessages) <= 1) {
                $title = TuquinhaEngine::generateShortTitle($text);
                if ($title) {
                    Conversation::updateTitle($conversationId, $title);
