...
class TuquinhaEngine
{
    ...
    /**
     * Gera resposta com streaming SSE — envia tokens conforme chegam da OpenAI.
     * Emite eventos SSE: "token" (pedaço de texto), "done" (resposta completa + tokens usados), "error".
     * Retorna o conteúdo completo acumulado (ou null em caso de erro).
     */
    public function streamResponseWithContext(array $messages, ?string $model = null, ?array $user = null, ?array $conversationSettings = null, ?array $persona = null): ?array
    {
        $configuredModel = Setting::get('openai_default_model', AI_MODEL);
        $modelToUse = $model ?: $configuredModel;
        ...
        // Streaming só suportado para OpenAI por enquanto
        if ($this->isClaudeModel($modelToUse)) {
            // Fallback: gera tudo e emite de uma vez
            $result = $this->callAnthropicClaude($messages, $modelToUse, $user, $conversationSettings, $persona, null);
            $this->emitSSE('token', ['text' => $result['content']]);
            $this->emitSSE('done', ['content' => $result['content'], 'total_tokens' => $result['total_tokens']]);
            return $result;
        }

        return $this->callOpenAIStreaming($messages, $modelToUse, $user, $conversationSettings, $persona);
    }

    private function callOpenAIStreaming(array $messages, string $model, ?array $user, ?array $conversationSettings, ?array $persona): ?array
    {
        $configuredApiKey = Setting::get('openai_api_key', AI_API_KEY);

        if (empty($configuredApiKey)) {
            $this->emitSSE('error', ['message' => 'API key não configurada.']);
            return null;
        }
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_WRITEFUNCTION => function ($ch, $data) use (&$fullContent, &$totalTokens, &$httpCodeRef, &$errorBuffer) {
                $httpCode = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
                $httpCodeRef = $httpCode;
        ...
        if ($httpCodeRef !== 200 || $fullContent === '') {
            if ($fullContent === '') {
                $errMsg = $errorBuffer !== '' ? $errorBuffer : ($curlError !== '' ? $curlError : 'Sem resposta da IA.');
                $this->emitSSE('error', ['message' => $errMsg]);
                return null;
            }
        }

        $this->emitSSE('done', ['content' => $fullContent, 'total_tokens' => $totalTokens]);
